...
import { Loader2, Eye, Save, Globe, GlobeOff, Plus, Trash2, Copy, ChevronUp, ChevronDown, Type, Image as ImageIcon, Video, Star, Clock, HelpCircle, ShoppingCart, Phone, Link2, FileText, Layout, Separator as SepIcon, MoveVertical, Code } from 'lucide-react'
...
const BLOCK_TYPES = [
  ...
  { type: 'purchase', label: 'Purchase Section', icon: ShoppingCart },
]
...
function getDefaultContent(type: string): Record<string, any> {
  switch (type) {
    ...
    case 'purchase': return { showPrice: true, buttonText: 'Purchase Now' }
    default: return {}
  }
}
...
function BlockPreview({ block }: { block: BuilderBlock }) {
  ...
  switch (type) {
    ...
    case 'purchase': return <div className="p-4 bg-muted rounded text-center"><div className="text-lg font-bold">Purchase Section</div><button className="mt-2 px-4 py-2 bg-primary text-white rounded">{content.buttonText}</button></div>
...
function BlockSettings({ block, onChange }: { block: BuilderBlock; onChange: (content: Record<string, any>) => void }) {
  ...
  switch (type) {
    ...
    case 'purchase': return (
      <div className="space-y-3">
        <h3 className="font-semibold text-sm">Purchase Section Settings</h3>
        <div className="flex items-center gap-2"><input type="checkbox" checked={content.showPrice} onChange={(e) => update('showPrice', e.target.checked)} /><label className="text-xs">Show Price</label></div>
        {renderField('buttonText', 'Button Text')}
      </div>
    )
    ...
    <div className="max-w-4xl mx-auto py-8 space-y-0">
      {blocks.map((block) => {
        ...
        switch (type) {
          ...
          case 'purchase': return (
            <section key={block.id} className="py-12 px-4 bg-muted/30">
              <div className="max-w-md mx-auto text-center">
                {content.showPrice && product && <div className="text-3xl font-bold mb-2">${product.price}</div>}
                <a href={getCtaUrl()} className="inline-block px-8 py-4 bg-primary text-primary-foreground rounded-xl font-bold text-lg hover:opacity-90 transition-opacity">{content.buttonText || getCtaLabel()}</a>
              </div>
            </section>
          )
          default: return null
